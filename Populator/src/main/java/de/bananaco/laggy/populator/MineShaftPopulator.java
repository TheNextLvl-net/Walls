...
public class MineShaftPopulator extends BlockPopulator {
    ...
    public void populate(World world, Random rand, Chunk chunk) {
        ...
        if (!canPlace(start))
            return;
        // System.out.println("Generating mineshaft at "+start.getX()+","+start.getY()+","+start.getZ());
        // now the populator begins
        int up = climb ? 1 : 0;
        Set<Block> wood = new HashSet<Block>();
        Set<Block> fence = new HashSet<Block>();
        Set<Block> rails = new HashSet<Block>();
        Set<Block> air = new HashSet<Block>();
        for (int i = 0; i < 8; i++) {

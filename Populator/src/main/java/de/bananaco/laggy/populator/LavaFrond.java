...
/**
 * A LAVA populator. Edited from GlowstonePopulator
 * 
 * @author Markus 'Notch' Persson
 * @author iffa
 * @author Nightgunner5
 */
public class LavaFrond extends BlockPopulator {
    // Variables
    ...
    /**
     * Populates a chunk with LAVA. Easily configurable (but results in
     * more rare LAVA) by modifying the suitable()-method. You could also
     * Remove the suitable()-check to have LAVA anywhere.
     * 
     * @param world
     *            World
     * @param random
    ...
    public void populate(World world, Random random, Chunk source) {
        for (int i = 0; i < 2; i++) {
            ...
            if (block.getRelative(BlockFace.DOWN).getTypeId() != Material.OBSIDIAN.getId()) {
                return;
            }
            ...
            for (int j = 0; j < 1500; j++) {
                ...
                for (BlockFace face : faces) {
                    if (current.getRelative(face).getTypeId() == Material.LAVA.getId()) {
                        count++;
                    }
                }

                if (count == 1) {
                    current.setTypeId(Material.LAVA.getId());
                }
            }
        }
    }
    ...
    /**
     * Checks if the given Y-coordinate is "suitable" for LAVA generation. <br />
     * <br />
     ...
     *            Y-coordinate
     * 
     * @return true if the given Y-coordinate is suitable for LAVA generation
     */

...
public class Populator_Lake_And_Creek extends BlockPopulator {
    ...
    private boolean checkBlockIsOnBorderOfSlice(Block block, Set<Block> slice) {
        ...
        if (slice.contains(block.getRelative(faces[0])) && slice.contains(block.getRelative(faces[1])) && slice.contains(block.getRelative(faces[2])) && slice.contains(block.getRelative(faces[3]))) {
            return false;
        }
        return true;
    }

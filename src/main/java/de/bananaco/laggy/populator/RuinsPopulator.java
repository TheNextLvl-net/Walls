...
public class RuinsPopulator extends BlockPopulator {
    ...
    public static final int RUINS_CHANCE = 12;
    public static final Material[] RUINS_MATERIAL = { Material.COBBLESTONE, Material.MOSSY_COBBLESTONE };
    public static final BlockFace[] directions = new BlockFace[] { BlockFace.NORTH, BlockFace.SOUTH, BlockFace.EAST, BlockFace.WEST };
        ...
        while (random.nextInt(100) < RUINS_CHANCE && ruins < MAX_RUINS) {
            ...
            while (height > 0 && 0 <= x && x < 16 && 0 <= z && z < 16) {
                ...
                for (; y < startY + height; y++) {
                    source.getBlock(x, y, z).setType(RUINS_MATERIAL[random.nextInt(RUINS_MATERIAL.length)]);
                }

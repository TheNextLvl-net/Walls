...
public class HiddenBox extends BlockPopulator {
    ...
    public boolean isHiddenBox(World world, int x, int z) {
        ...
        if (world.getBlockTypeIdAt(x, 0, z) == Material.BEDROCK.getId() && world.getBlockTypeIdAt(x, 1, z) == Material.GLOWSTONE.getId() && world.getBlockTypeIdAt(x, 2, z) == Material.AIR.getId() && world.getBlockTypeIdAt(x, 3, z) == Material.AIR.getId() && world.getBlockTypeIdAt(x, 4, z) == Material.AIR.getId() && world.getBlockTypeIdAt(x, 5, z) == Material.BEDROCK.getId())
            return true;
        return false;
    }
